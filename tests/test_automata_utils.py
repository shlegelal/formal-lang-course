...
from project.utils.automata_utils import AutomataUtilsError, build_dfa_by_redex, build_nfa_by_graph
from project.utils.graph_utils import generate_labeled_two_cycles_graph, get_graph
...
def test_build_nfa_by_labeled_two_cycles_graph(
    ...
    expected_graph = generate_labeled_two_cycles_graph((first_cycle_num, second_cycle_num), ("a", "b"))
...
@pytest.mark.parametrize(
    "graph_name",
    load_test_res("test_build_nfa_by_dataset_graph")
)
def test_build_nfa_by_dataset_graph(
        graph_name: str
):
